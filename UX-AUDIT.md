...
## 2026-07-10 serial-fixer merge - round 09 extreme-zoom containment

Read all 40 findings in `orchestrator/_work/round-09/findings-0.json` through `findings-5.json`. No distinct page/issue was new after deduplication: the moisture snapshot, dashboard generation/command authority, camera telemetry range, camera-reading detail, and calibration/sensor authority reports refine the existing open campaigns. The two Grapes/manual-zone and sync-group reports deduplicate into Watering-behavior (DO NOT FIX) and were not changed.

| Page / area | Severity | Status | Category | Resolution / RCA |
|---|---|---|---|---|
| forecast comparison | med | fixed (`4034c11`, `21bfa08`, `49a495d`) | 200% and 400% zoom reflow | Tabs stack below 260px effective width; filters, comparison text, summary cards, forecast rows, and shared mobile navigation can shrink/wrap without page overflow. Authenticated live Playwright measured `scrollWidth == clientWidth` at both 195px and 98px. |
| moisture-sim | high/med | open - broader RCA | duplicate waves, mixed generations, stale schedule and unbounded failure | Existing atomic all-zone display snapshot campaign. The safe fix needs one read-only generation-bearing payload and one client owner; the schedule generator and watering predictor remain out of scope. |
| dashboard and Map | high/med | open - broader RCA | validated generation, command authority, bounded polling and stale provenance | Existing shared snapshot-state campaign. Map already fails closed; Dashboard still requires a complete typed authority envelope and generation contract before controls can be enabled. |
| cam-device telemetry API/UI | high/med | open - overlapping reporting fix | half-open T-separated range, query validation, freshness and retention | Existing read-only reporting campaign in dirty `database.py`; do not absorb unrelated active database work or reinterpret naive timestamps piecemeal. |
| camera reading detail | high/med | open - overlapping work | held-value truth, schema/failure classification, Retry, landmark, mobile targets and provenance | Existing coordinated change in dirty `cam_reading.html`. The page-local recovery and truthfulness work must land with the active camera-detail edits rather than commit unrelated work. |
| calibration / sensor reporting | high/med | open - broader RCA | immutable active revision, physical identity, observation provenance, drift and freshness | Existing versioned calibration/sample serializer campaign. The current endpoints cannot support an honest UI-only join, and legacy authority must be labeled unknown rather than invented. |

The fix is template-only (`forecast_merged.html` and shared `_mobilenav.html`). No backend `.py`, irrigation balance/credit, schedule generation, runtime, precipitation, valve, MAD, or watering configuration changed. Each deployment used timestamped remote backups, public `/login` smoke testing, and SHA-256 parity. Thirty-two non-watering high/medium raw findings remain grouped into the five coordinated campaigns above.

## 2026-07-10 round-10 serial-fixer merge

Read all 36 findings in `orchestrator/_work/round-10/findings-0.json` through `findings-5.json`. No distinct page/issue was new after deduplication. The reports refine the existing moisture atomic-snapshot, dashboard authority/generation, camera telemetry/reporting-time, camera-reading provenance/detail, calibration/sensor authority, and strict-CSP campaigns. The explicit manual-Grapes moisture contradiction and the automatic schedule entry for that manual zone are both watering-behavior flags; they remain in the DO NOT FIX section and no scheduling, balance, runtime, valve, MAD, precipitation, or watering configuration code was changed.

| Page | Severity | Status | Category | Expected vs Actual | Resolution / RCA |
|------|----------|--------|----------|--------------------|------------------|
| audit | high | fixed (`493f762`) | strict CSP compatibility | Inline CSS and JavaScript made the page unusable under nonce-free `script-src 'self'; style-src 'self'`. | The served page now references same-origin `audit.css` and `audit.js`; live authenticated HTML contains no inline style/script block. |
| audit | high | fixed (`493f762`) | semantic fallback / failure state | Initial HTML had an empty table and depended entirely on JavaScript. | The route now server-renders the complete current summary and 25-row table; Refresh is progressive enhancement and failed refresh preserves the baseline. |
| authenticated shared shell | high | open - broader RCA | page-family CSP dependency | Shared/page-family inline controllers, handlers, and presentation still prevent strict enforcement across the authenticated site. | Existing staged CSP extraction campaign; the isolated audit-page fix does not claim site-wide readiness. |
| moisture-sim | high/med | open - broader RCA | duplicate initialization, non-atomic generations, stale fallback, and schedule failure | Per-zone requests and duplicate owners can mix generations and leave schedule state stale or ambiguous. | Existing atomic display-snapshot campaign. A safe fix needs one read-only generation envelope and one client owner without altering the schedule generator or watering predictor. |
| dashboard / Map | high/med | open - broader RCA | polling authority, command enablement, schema validation, and provenance | Out-of-order or partial responses can overwrite newer state or leave operational controls authoritative. | Existing shared generation/authority adapter campaign. Map fails closed; Dashboard still requires the coordinated adapter and bounded polling. |
| cam telemetry and reporting history | high/med | open - overlapping reporting fix | T-separated rolling cutoff, strict range contract, and freshness | One-hour camera telemetry and related soil/rain helpers use space-separated lexical cutoffs against T-separated stored timestamps. | Display/report queries only, but `database.py` contains substantial unrelated in-progress work and the common ZoneInfo-aware half-open reporting migration remains the safe landing path. |
| camera reading detail | high/med | open - overlapping work / broader RCA | stale-reading truthfulness, recovery states, announcements, schema validation, reflow, and targets | Held locks appear in the field dump and failures collapse to a false pruned-record message; page-local accessibility/reflow gaps remain. | Existing common camera serializer/detail campaign. `cam_reading.html` and shared navigation contain active unrelated changes, so this pass did not absorb or deploy them. |
...
| Page / area | Severity | Status | Category | Resolution / RCA |
|---|---|---|---|---|
| forecast | med | fixed (`065491d`) | overlapping request ownership | One module-level controller aborts the prior load, and a monotonically increasing sequence prevents superseded success, error, or cleanup paths from mutating the page. Template-only display fix. |
...
The fix is template-only; no backend `.py`, watering engine, balance/credit, schedule generator, valve, runtime, precipitation, MAD, or watering configuration code was changed. Deployment used backup `forecast_merged.html.bak.round12-20260710-232510`; authenticated live `/forecast`, `/api/forecast`, and `/api/schedule-7day` returned 200, public `/login` returned 200, the service was active, and post-deploy SHA-256 parity passed. The browser runtime was not exposed, so the raw auditor's authenticated reproduction evidence was retained and no fresh browser-DOM claim is made.

## 2026-07-10 round-13 serial-fixer merge

Read all 41 findings in `orchestrator/_work/round-13/findings-0.json` through `findings-5.json`, including the zero-finding command-authority file. No distinct page/issue was new after deduplication. Thirty-eight high/medium reports refine the existing atomic moisture-snapshot, reporting-time/freshness, camera-reading detail, strict-CSP, regression-inference, and CNN metric-provenance campaigns.

| Page / area | Severity | Status | Category | Resolution / RCA |
|---|---|---|---|---|
| moisture-sim | high/med | open - broader RCA | atomic generation, duplicate owners, stale cache/schedule, bounded failure and snapshot identity | Existing atomic all-zone display-snapshot campaign. The safe fix requires one read-only generation-bearing envelope and one client owner. The Grapes automatic-schedule claim is the already-logged watering/control contradiction and was not changed. |
| cam-device, rain-events, sensor-history | high/med | open - overlapping reporting fix | half-open local-time ranges, strict parameters, freshness, retention and mobile coverage | Existing ZoneInfo-aware reporting-time migration. `database.py` contains unrelated in-progress work; a piecemeal cutoff change would leave inconsistent timestamp contracts across reporting consumers. |
| camera reading detail | high/med | open - overlapping work | archive lookup, held-lock provenance, typed failures, Retry, image/CNN recovery, landmarks and targets | Existing coordinated serializer/detail-page campaign in dirty camera files. Archive and live-ring identity plus carried-lock provenance need one durable API contract. |
| costs, water-usage, authenticated shell | high/med | open - broader RCA | strict CSP, semantic fallback, stale refresh and safe rendering | Existing staged page-family CSP extraction/server-rendered fallback campaign. Enforcing nonce-free CSP before shared and page assets are externalized would break authenticated pages. |
| cam/regression | high/med | open - broader RCA | bounded cached inference, pagination and benchmark provenance | Existing coordinated API/client campaign. Persisted model-versioned evaluation and a strict paginated contract are required; repeated live inference cannot be repaired honestly in presentation alone. |
...
## 2026-07-10 round-15 serial-fixer merge

Read all 44 findings in `orchestrator/_work/round-15/findings-0.json` through `findings-5.json`; 40 were high/medium. Two distinct Test Audit issues were added and fixed. The two Grapes/manual-zone reports deduplicate into Watering-behavior (DO NOT FIX) and were not changed.

| Page / area | Severity | Status | Category | Resolution / RCA |
|---|---|---|---|---|
| cam/test-audit | high | fixed (`0e5b5de`) | stale success after failure | Every load now clears the prior benchmark array, count, cards, and controls before requesting data; every failure clears them again and exposes only the retryable unavailable state. Authenticated live Playwright changed a forced-500 refresh from 83 stale cards to zero cards and a blank count. |
| cam/test-audit | med | fixed (`31fef84`) | duplicate image request amplification | Removed the per-card warm GET; each card retains its existing image element request. Authenticated live Playwright rendered all 83 current cards after deployment. |
| moisture-sim | high/med | open - broader RCA | atomic snapshot ownership, stale writes, bounded failures, and history states | Existing atomic all-zone display-snapshot campaign. One generation-bearing read-only envelope and one client owner are required; do not change schedule generation or the watering predictor. |
| dashboard and map | high/med | open - broader RCA | polling generation, schema authority, fail-closed commands, and shared provenance | Existing command-authority snapshot campaign. Controls and displayed freshness must commit from one validated generation; this spans both pages and their common API contract. |
| cam-device and rain-events | high/med | open - overlapping reporting fix | T-separated rolling bounds, strict parameters, retention, timezone, and coverage | Existing ZoneInfo-aware reporting-time campaign. `database.py` contains unrelated in-progress work, and isolated cutoff edits would leave the endpoint family internally inconsistent. |
| camera reading detail | high/med | open - overlapping work / broader RCA | durable identity, held-lock provenance, archive lookup, typed recovery, image integrity, and reflow | Existing immutable-frame serializer/detail campaign in dirty camera files. A template cannot invent durable frame/revision joins or authoritative provenance. |
| cam/test-audit and regression APIs | high/med | open - broader RCA | exhaustive persisted evaluation, bounded inference, model/label provenance, and timeout contract | Existing model-versioned persisted benchmark campaign. The remaining work requires a durable evaluation artifact/job rather than more synchronous inference in a page request. |
| cam/cnn-report and cam/quality | high/med | open - broader RCA | exhaustive outcomes, metric reconciliation, oracle semantics, null handling, denominators, confidence and identity | Existing typed camera metric/provenance campaign. Separate current aggregates do not expose enough authority to invent a truthful partition or reconcile the pages in presentation code. |

Thirty-eight non-watering high/medium raw findings remain represented by the six coordinated campaigns above. Both template-only checkpoints were deployed separately after timestamped remote backups, public `/login` smoke checks, authenticated live Playwright checks, and server/local SHA-256 parity. No backend `.py`, irrigation balance/credit, schedule generator, valve, runtime, precipitation, MAD, or watering configuration file was changed.

## 2026-07-11 round-16 serial-fixer merge

Read all 45 findings in `orchestrator/_work/round-16/findings-0.json` through `findings-5.json`, including the zero-finding camera-identity file. Thirty-five were non-watering high/medium findings. No distinct page/issue was new after deduplication: every report refines one of the six coordinated campaigns already open below. The seven high findings marked as watering behavior cover the live Grapes automatic-mode contradiction and dashboard/Map command-authority, poll-race, and contradictory-state risks; they remain in Watering-behavior (DO NOT FIX) and were not changed.

| Page / area | Severity | Status | Round-16 refinement | Resolution / RCA |
|---|---|---|---|---|
| moisture-sim | high/med | open - broader RCA | duplicate initialization, stale-generation overwrite, non-atomic cross-API state, partial/malformed failures, and invented config defaults | Existing atomic all-zone display-snapshot campaign. A generation-bearing read-only envelope and single client owner are required; do not change schedule generation or prediction logic. |
| dashboard and map | med display provenance; high watering/control risk | logged-only / broader RCA | cached data labeled live, sensor-test poll validation, command authority, stale poll ownership, and contradictory command state | Display-only freshness work depends on an authoritative shared API contract. The seven command/control findings are quarantined as watering behavior and require the watering-control change process. |
| cam-device, rain-events, and sensor-history | high/med | open - broader RCA | T-separated rolling cutoffs, strict ranges, ZoneInfo interval semantics, retention/freshness, and sampling reconciliation | Existing ZoneInfo-aware reporting-time campaign. Isolated cutoff edits would leave the endpoint family internally contradictory, and `database.py` contains unrelated in-progress work. |
| cam/test-audit and regression | high/med | open - broader RCA | exhaustive pagination, persisted evaluation, bounded inference, typed failures, request ownership, and model/label/result provenance | Existing model-versioned persisted benchmark campaign. A durable evaluation artifact/job is required; presentation cannot manufacture immutable run identity. |
| cam/cnn-report and cam/quality | high/med | open - broader RCA | exhaustive outcomes, cross-page reconciliation, null/error truth, oracle authority, timestamp/window provenance, and accessible metric semantics | Existing typed camera metric/provenance campaign. The current APIs do not expose enough authority for a truthful display-only partition. |
| camera reading detail | high/med | open - broader RCA | no new executable evidence in round 16 | Existing immutable-frame serializer/detail campaign remains open; the dedicated auditor could not run because its browser runtime was unavailable. |

No product code, backend `.py`, irrigation balance/credit, schedule generator, valve, runtime, precipitation, MAD, or watering configuration file was changed. No deployment was warranted. The director should schedule the six coordinated RCA campaigns rather than treating these as isolated template defects.

## 2026-07-11 round-17 serial-fixer merge

Read all 46 findings in `orchestrator/_work/round-17/findings-0.json` through `findings-5.json`; 38 were non-watering high/medium findings. No distinct page/issue was new after deduplication: every report refines one of the six coordinated campaigns already open below. Three high findings marked as watering behavior cover the live Grapes automatic-mode contradiction and dashboard/Map command-authority failure; they remain in Watering-behavior (DO NOT FIX) and were not changed.

| Page / area | Severity | Status | Round-17 refinement | Resolution / RCA |
|---|---|---|---|---|
| moisture-sim | high/med | open - broader RCA | duplicate initialization, stale-generation overwrite, non-atomic cross-API state, partial/malformed failures, cache mixing, invented defaults, and stale history ownership | Existing atomic all-zone display-snapshot campaign. A generation-bearing read-only envelope and single client owner are required; do not change schedule generation or prediction logic. |
| dashboard and map | med display provenance; high watering/control risk | logged-only / broader RCA | contradictory sensor calibration provenance, missing shared generation identity, cached values labeled live, and fail-closed command authority | Display-only freshness and sensor truth depend on an authoritative shared API contract. The two command/control findings are quarantined as watering behavior and require the watering-control change process. |
| cam-device, rain-events, and sensor-history | high/med | open - broader RCA | rolling cutoff validation, ZoneInfo interval semantics, retention/freshness, sampling totals, and shared acquisition boundaries | Existing ZoneInfo-aware reporting-time campaign. Isolated endpoint edits would leave the reporting family internally contradictory, and `database.py` contains unrelated in-progress work. |
| cam/test-audit and regression | high/med | open - broader RCA | exhaustive persisted evaluation, bounded inference, immutable identity, request ownership, integrity validation, and model/label/run provenance | Existing model-versioned persisted benchmark campaign. A durable evaluation artifact/job is required; presentation cannot manufacture immutable run identity or complete denominators. |
| cam/cnn-report and cam/quality | high/med | open - broader RCA | exhaustive outcome partition, oracle-call semantics, evaluation authority, cross-page identity, uncertainty, definitions, and null truth | Existing typed camera metric/provenance campaign. The current APIs do not expose enough authority for a truthful display-only partition or reconciliation. |
| camera reading detail | high/med | open - broader RCA / overlapping work | durable frame/revision identity, archive fallback, held-lock provenance, typed recovery, image/CNN ownership, landmarks, zoom, and timestamp provenance | Existing immutable-frame serializer/detail campaign in dirty camera files. The page cannot truthfully recover archive identity or distinguish carried values without the coordinated serializer contract. |

No product code, backend `.py`, template, irrigation balance/credit, schedule generator, valve, runtime, precipitation, MAD, or watering configuration file was changed. No deployment was warranted. The director should schedule the six coordinated RCA campaigns rather than treating these as isolated display defects.

## 2026-07-11 round-18 serial-fixer merge

Read all 31 findings in `orchestrator/_work/round-18/findings-0.json` through `findings-5.json`, including the zero-finding review file; 26 were non-watering high/medium findings. No distinct page/issue was new after deduplication: every report refines an existing coordinated campaign. The Grapes/manual-only `soil_dry` history attribution is explicitly marked watering behavior and deduplicates into the Watering-behavior (DO NOT FIX) section above.

| Page / area | Severity | Status | Round-18 refinement | Resolution / RCA |
|---|---|---|---|---|
| calibrate and sensor-history | high/med | open - broader RCA | active-vs-history calibration authority, observation freshness, immutable sensor identity, historical revision provenance, total ordering, drift-source semantics, and empty/loading truth | Existing calibration/sensor authority campaign. Truthful display requires an immutable revision and physical-sensor contract; isolated labels would manufacture provenance. |
| cam/archive | high/med | open - broader RCA | strict pagination validation, duplicate parameters, history restoration, thumbnail/lazy-load performance, long-task rendering, and partial-schema rejection | Existing durable camera archive/detail campaign. Coordinate the API contract, immutable archive identity, pagination state, and thumbnail delivery; camera/backend files contain unrelated in-progress work. |
| costs and authenticated shell | high/med | open - broader RCA | strict CSP, semantic server fallback, Chart.js failure isolation, URL-scheme validation, accessible daily data, and table semantics | Existing staged page-family CSP/server-rendered fallback campaign. Shared shell extraction and a semantic baseline must land together; piecemeal enforcement would blank authenticated pages. |
| flow and water-usage | high/med | open - broader RCA | incompatible per-zone measurement populations, unresolved/open-event all-clear contradiction, and zero-based zone identifiers presented as user labels | Existing read-only meter/reporting authority campaign. The UI needs canonical event IDs, inclusion/exclusion reasons, and an authoritative active-event definition; relevant backend files are dirty with unrelated work. |
| cam/convergence | high/med | open - broader RCA | requested-vs-effective windows, lifetime coverage denominator, complete category partition, atomic as-of identity, timestamp provenance, and stratified confidence/source disclosure | Existing typed camera metric/provenance campaign. A versioned read-only snapshot contract is required before the UI can present checkable denominators and cross-page totals. |

Root-cause disposition: these are broader contract/campaign issues, not isolated display defects. A minimal template patch would conceal missing authority, leave related pages contradictory, or collide with active work. No product code, backend `.py`, template, irrigation balance/credit, schedule generator, valve, runtime, precipitation, MAD, or watering configuration file was changed. No deployment was warranted.

## 2026-07-11 round-19 serial-fixer merge

Read all 44 findings in `orchestrator/_work/round-19/findings-0.json` through `findings-5.json`; 38 were non-watering high/medium findings. Two distinct Camera Quality accessibility issues were fixed. The other 36 high/medium reports refine six existing coordinated campaigns. The manual-zone configuration-ownership finding is watering behavior and remains in the Watering-behavior (DO NOT FIX) section.

| Page / area | Severity | Status | Round-19 finding | Resolution / RCA |
|---|---|---|---|---|
| cam/quality | med | fixed (`337427e`) | Recent-read match cells had a blank column heading and symbol-only values. | The column is headed “CNN matched oracle”; each visual symbol now has equivalent screen-reader text (“Match” or “Mismatch”). |
| cam/quality | med | fixed (`80c74f4`) | Keyboard users had no direct route past tool navigation to main content. | Added a first-focusable, visible-on-focus skip link targeting the existing `main` landmark. |
| moisture-sim | high/med | open - broader RCA | manual-zone ownership, atomic generation, stale fallback, duplicate initialization, and independent failure states | The manual-zone configuration finding is watering behavior and was not changed. The display findings remain the atomic all-zone snapshot/single-client-owner campaign; do not alter schedule generation or prediction logic. |
| dashboard and map | high/med | open - broader RCA | poll generation ownership, schema validation, cached authority, sensor identity/freshness, and cross-page reconciliation | Existing shared snapshot and command-authority contract. A display-only patch cannot manufacture a common generation or authoritative command state. |
| cam-device, rain-events, and sensor-history | high/med | open - broader RCA | time-window filtering, parameter validation, retention/freshness, DST semantics, loading failure, and acquisition-boundary reconciliation | Existing ZoneInfo-aware reporting-time campaign. It requires one consistent range/acquisition contract across the reporting endpoints; `database.py` contains unrelated active work. |
| camera reading detail | high/med | open - broader RCA / overlapping work | durable identity, archive fallback, held-value provenance, typed metadata/image/CNN recovery, immutable timestamps, and nonvisual equivalence | Existing immutable-frame serializer/detail campaign in dirty camera files. Archive/live identity and accepted-state ownership must be fixed together rather than invented in the template. |
| cam/test-audit and regression | high/med | open - broader RCA | exhaustive persisted evaluation, synchronous inference, immutable run provenance, reconciliation, timeout, and cancellation | Existing model-versioned persisted benchmark campaign. A durable evaluation job/artifact is required; presentation cannot create complete immutable results. |
| cam/cnn-report and cam/quality | high/med | open - broader RCA | complete outcome partition, oracle-call semantics, snapshot identity, confidence equivalence, and mixed-version partial failure | Existing typed camera metric/provenance campaign. Current APIs do not expose the authority needed for truthful partitioning and cross-page reconciliation. |

Both template-only fixes were committed and deployed separately after remote pre-diff/hash checks and timestamped backups. The service restarted active, public `/login` returned 200, and post-deploy server/local SHA-256 parity passed. The required authenticated browser runtime was not exposed in this session, so the raw auditor's authenticated reproduction evidence plus local/live source checks were used; no fresh browser-DOM claim is made. No backend `.py`, irrigation balance/credit, schedule generator, valve, runtime, precipitation, MAD, or watering configuration file changed.
## 2026-07-11 serial-fixer merge - round 20 camera labels, flow truth, and reporting contracts

Read all 32 findings in `orchestrator/_work/round-20/findings-0.json` through `findings-5.json`, including the zero-finding `/cam` audit. Seven standalone page/issue refinements were newly folded into the backlog; the remaining high/medium reports deduplicate into the existing calibration/sensor authority, reporting-time/lineage, audit-health performance, and canonical water-reporting campaigns. The Grapes automatic-mode/schedule/scoring contradiction is Watering-behavior (DO NOT FIX) and was not changed.

| Page / area | Severity | Status | Category | Resolution / RCA |
|---|---|---|---|---|
| cam/labels | high | fixed (`6dd0e8d`) | count-envelope DOM XSS | Every returned count must be a finite non-negative integer before rendering or write controls can become available. A payload count cannot cross the existing summary HTML sink as markup. |
| cam/labels | med | fixed (`5095abf`) | nine-character label injection | The special nine-character formatter escapes both portions of the label before interpolation. |
| cam/labels | med | fixed (`a63adee`) | stale request ordering | A monotonically increasing load generation prevents an older success or failure from replacing the newest filter/load state. |
| cam/labels | med | fixed (`71565bb`) | duplicate actionable identity | Duplicate detection now keys only on actionable `file`, so timestamp variants cannot enable ambiguous mutations. |
| flow | high | fixed (`85fd488`) | false all-clear with unresolved events | Every unresolved event participates in the alert banner; age-based presentation no longer suppresses it into an all-clear. |
| flow / api/flow | high | open - backend overlap; client hardened (`9d19f8c`) | error envelope returned as HTTP 200 | Flow now rejects `error` payloads and clears stale display state. The API still needs a non-2xx response, but `dashboard.py` contains extensive unrelated active work and cannot safely be deployed from this pass. This is a broader clean-backend RCA item. |
| forecast comparison | med | fixed (`021bd67`) | crop ET mislabeled as generic ET | Rows displaying `etc_mm` now label the value `ETc`, distinguishing it from reference ET0. |
| calibrate / sensor-history APIs and pages | high/med | open - broader RCA | active revision, drift lineage, observation freshness, physical identity, ordering and null provenance | Existing versioned calibration/sample campaign. Current config, independent endpoint history rows, and index-only samples do not share an immutable revision or physical-sensor identity; a display patch cannot choose authority honestly. |
| forecast / forecast comparison | high/med | open - broader RCA | deep-link availability, shared schedule generation and complete lineage schema | The legacy route failure and cross-endpoint next-water cutoff need clean backend/reverse-proxy diagnosis and a common generated-at/revision contract. Partial-response validation cannot require snapshot metadata the API does not yet provide. |
| audit / api/audit | med | open - broader RCA | ignored query contract, stale failure state, unbounded rendering and repeated database scans | The endpoint is a fixed 25-table health snapshot, not audit history. Define supported parameters and bounded/cacheable aggregation first; then add fail-closed schema/render handling. Current `dashboard.py` and `database.py` are dirty with unrelated active backend work. |
| water usage / reconciliation / costs / flow | med | open - broader RCA | provisional current day, population boundaries, event identity and snapshot freshness | Existing canonical water-reporting campaign. The surfaces lack shared half-open boundaries, event links, ledger method/version, open-event semantics, and snapshot capture time. Isolated labels would manufacture reconciliation authority. |

All six fixed findings were committed and deployed one checkpoint at a time with timestamped remote backups, service restarts, public `/login` smoke checks, and local/server SHA-256 parity. Authenticated Playwright and live API responses reproduced the 509-label payload and three unresolved flow events; the final Flow page no longer reports an all-clear. No backend Python, irrigation balance, schedule generation, runtime, valve, MAD, precipitation, or watering configuration file was changed.

## 2026-07-11 round-21 serial-fixer merge - typed reporting failures

Read all 21 findings in `orchestrator/_work/round-01/findings-0.json` through `findings-5.json`, including the zero-finding Forecast file. Three page/issue refinements were new after deduplication; the Flow, Audit, CNN Report, calibration/sensor, liveness/schedule, and favicon reports were already represented. No finding was marked as watering behavior.

| Page / area | Severity | Status | Category | Resolution / RCA |
|---|---|---|---|---|
| api/flow | med | fixed (`1a08bbc`) | typed HTTP failure contract | Reporting failures now return HTTP 503 with a stable `FLOW_UNAVAILABLE` envelope and no exception detail. The existing Flow client rejects non-2xx responses, clears operational claims, and exposes Retry. `dashboard.py` changed only this read-only display API. |
| water-usage OCR audit | med | open - broader RCA | query failure represented as empty success | New refinement of the canonical water-reporting contract: the API must return typed non-2xx failure and the chart must distinguish unavailable from an empty window. Coordinate with the dirty meter/reporting work so failure truth and population authority land together. |
| cam/labels reprocess status | med | open - broader RCA | status-read failure represented as stopped | The endpoint and polling client need a typed unavailable state that preserves unknown job ownership and does not re-enable mutation controls. Treat this as a job-authority RCA, not an isolated all-clear label change. |
| cam/labels propagate status | med | open - broader RCA | status-read failure represented as conflict-free | The endpoint and every consumer need a typed unavailable state; failure must not be rendered as empty counts/no conflicts. Coordinate with the same camera job-authority contract. |
| api/audit and Audit | med | open - broader RCA | ignored parameters, repeated scans, overlapping request ownership | Existing bounded audit-snapshot campaign. A short-lived single-flight snapshot, explicit query contract, and latest-request ownership must land together; current backend reporting files contain unrelated work. |
| calibrate / sensor APIs and pages | high/med | open - broader RCA | active revision, drift lineage, identity and observation freshness | Existing versioned calibration/sample campaign. The UI cannot truthfully infer an active revision from independent history rows. |
| health / schedule telemetry | med | open - broader RCA | public liveness and route-local timing attribution | Existing infrastructure campaign; this is observability rather than a display-only correction. |

CNN Report's insights integration, persistent loading announcement, and controlled parse/schema failures remain fixed (`8690126`, `27227e0`, `4bbd84a`). The Flow checkpoint was compiled, backed up, deployed, restarted, authenticated-curl checked, `/login` smoked, and SHA-256 verified against the server. The in-app browser runtime was not exposed, so no fresh browser-DOM verification is claimed.
