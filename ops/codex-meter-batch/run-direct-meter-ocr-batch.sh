...
mkdir -p "$RUN"
chmod 700 "$STATE" "$STATE/runs" "$RUN"
...
flock -n 9 || { write_status skipped reason same_job_active; exit 0; }
exec 8>/home/john/.local/share/codex-jobs/global.lock
flock -n 8 || { write_status skipped reason other_codex_job_active; exit 0; }

if [[ ! -f "$SUMMARY" ]]; then
  write_status blocked reason missing_holdout_summary
  exit 0
fi
...
if (( FALSE_ACCEPTS > 0 )); then
  write_status blocked reason verified_holdout_false_accept false_accept_count "$FALSE_ACCEPTS"
  exit 0
fi
