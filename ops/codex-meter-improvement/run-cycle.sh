...
James's product goal includes the complete historical sprinkler-event backlog, not only a sample benchmark or future events. Inventory every eligible historical watering event with archived boundary imagery and classify it as directly verified, OCR-recoverable, missing imagery, or unresolved. Design the workstream so successful guarded recognition can resume across later cycles from durable checkpoints, processing previously unresolved events without repeating completed calls. The benchmark is the gate to scalable recovery, not the final deliverable.

The next required non-repeating experiment follows the 20260721 benchmark failure analysis. Reuse its inventory, packs, and checkpoints. Compare verified events 826 and 905 using separate image attachments rather than contact sheets; rank candidate frames by saturation, sharpness, LCD contrast, and glare diversity; preserve minimally processed color crops; and request per-digit segment/candidate evidence that deterministic chronological carry reconciliation can consume. Do not repeat the overexposed grayscale contact-sheet presentation. Treat this as the priority unless read-only evidence proves an exact prerequisite is unavailable.
PROMPT
)
...
After a reader configuration passes its chronological zero-false-accept gate, use the remaining runtime to recover the historical backlog in bounded batches and checkpoint per-event attempts, image provenance, model/prompt/preprocessing versions, candidate readings, validation decisions, and retry state under $LAB. Subsequent cycles must resume that queue. Promote accepted historical event boundaries and gallons to the canonical event-authority/Water Usage path when the applicable backup, database, idle-valve, test, and display-contract gates pass. Never label duration-times-GPM reconstruction as OCR, never manufacture a reading to reach 100% coverage, and leave genuinely unreadable or missing-image events explicitly unresolved with a reason.

For the immediate separate-attachment experiment, use the already generated verified-event assets and avoid rebuilding the 970-event inventory. Select several exposure-diverse start and end frames per event, attach them individually at their native or minimally cropped resolution, and keep color information. Emit partial per-position candidates or seven-segment visibility when a full digit is uncertain, then let deterministic code enumerate only unique monotonic suffix/carry solutions within physical bounds. Score event 826 and event 905 independently and preserve raw model outputs. If both safely reject, test one restrained local-normalization/deglare variant on the same attachments; do not create another contact sheet.
