...
# After this many consecutive status failures, escalate WARNING → ERROR.
...
class IrrigationEngine:
    ...
    def __init__(self, config: dict, weather: WeatherClient,
        ...
        # Shared secret for /api/reboot — must match firmware src/config.h
        ...
        # Two HTTP sessions:
        #   _esp32_status: retries=0 — fail-fast on /api/status to avoid
        #     bursts that exhaust the chip's lwIP TIME_WAIT pool (#10).
        #     Resilience comes from the cache + debounce, not retry storms.
        #   _esp32_cmd:    retries=3 — valve open/close are too important
        #     to drop on a single transient error.
        #   _esp32_manual: retries=0 — dashboard button clicks must return
    ...
    def reboot_esp32(self) -> tuple[bool, str]:
        ...
        state but still answering HTTP — avoids a physical USB trip.
        """
    ...
    def calculate_weather_scale(self, allow_weather_fetch: bool = True) -> dict:
        ...
        # Humidity delta: lower humidity → more evaporation → water more
        humidity_delta = self._wa_base_hum - humidity

        # Temperature delta: higher temp → more evaporation → water more
        temp_delta = (temp_f - self._wa_base_temp) * self._wa_temp_factor

        # Rain delta: recent rain → less watering needed
    ...
    def is_in_watering_window(self, zone_id: int) -> bool:
        ...
        # Morning window — all zones
        if self.window["start"] <= current_time <= self.window["end"]:
            return True

        # Evening window — drip zones only
    ...
    def evaluate_zone(self, zone_id: int, soil_pct: float,
        ...
        # Dormant season — skip everything
        if season_idx < 0:
            return self._decision("skip", zone_id, "Dormant season — no irrigation",
                                  {})
        ...
        # ── SKIP CONDITIONS (ordered by impact) ──

        # 1. Soil already wet enough
        if soil_pct >= zone["wet_target"]:
            return self._decision("skip", zone_id,
                                  f"Soil at {soil_pct:.0f}% (target {zone['wet_target']}%) — already wet",
                                  conditions)

        # 2. Soil above dry trigger — no need yet
        if soil_pct > zone["dry_trigger"]:
            return self._decision("skip", zone_id,
                                  f"Soil at {soil_pct:.0f}% > trigger {zone['dry_trigger']}% — not dry enough",
                                  conditions)
        ...
        # 7. Budget tightening — late in billing cycle and in expensive tier
        budget = self.billing.should_tighten_budget()
        if budget["tighten"]:
            # Still water if soil is critically dry (< 50% of trigger)
            critical = zone["dry_trigger"] * 0.5
            if soil_pct > critical:
                return self._decision("skip", zone_id,
        ...
        if not self.is_in_watering_window(zone_id):
            return self._decision("wait", zone_id,
                                  "Outside watering window — will water at next window",
                                  conditions)

        # ── WATER ──
        return self._decision("water", zone_id,
                              f"Soil at {soil_pct:.0f}% < trigger {zone['dry_trigger']}% — watering",
                              conditions)
    ...
    def _evaluate_active_zone(self, zone_id: int, soil_pct: float,
        ...
        # Stop if soil target reached
        if soil_pct >= zone["wet_target"]:
            return self._decision("close", zone_id,
                                  f"Target reached: soil at {soil_pct:.0f}% >= {zone['wet_target']}%",
                                  conditions)
        ...
        if elapsed >= max_sec:
            return self._decision("close", zone_id,
                                  ...
                                  f"(base {zone['max_runtime_min']}min × {scale}%)",
                                  conditions)
    ...
    def start_zone_watering(self, zone_id: int, soil_before: float,
        ...
        if scale_pct == 0 and reason != "manual":
            log.info("Zone %d: weather scale 0%% — skipping (rain: %.1fmm)",
                     zone_id, ws["rain_last_24h_mm"])
            return False
        if scale_pct == 0 and reason == "manual":
            log.info("Zone %d: weather scale 0%% but manual override — running anyway",
                     zone_id)
            scale_pct = 100  # full runtime for manual

        zone = self.zones[zone_id]
        adjusted_min = zone["max_runtime_min"] * scale_pct / 100.0
        log.info("Zone %d: weather scale %d%% → runtime %.0fmin (base %dmin) "
                 "[temp_Δ=%.1f hum_Δ=%.1f rain_Δ=%.1f]",
    ...
    def log_skip_event(self, zone_id: int, reason: str, conditions: dict):
        """Log that we skipped watering — estimates what WOULD have been used."""
        zone = self.zones[zone_id]
    ...
    def run_cycle(self):
        """Main decision cycle — evaluate all zones and act."""
        ...
        for zone in installed_zones:
            ...
            if sensor_idx is None:
                ...
                log.warning(
                    "Zone %d (%s) is installed but has no valid soil sensor; "
                    "skipping automatic watering decision",
                    zone["id"], zone["name"],
                )
                continue
        ...
        for zone in installed_zones:
            zid = zone["id"]
            if zid in invalid_sensor_zones:
                reason = "No valid soil sensor configured"
                decision = self._decision("skip", zid, reason, {})
                actions.append(decision)
                skip_reasons[reason] = skip_reasons.get(reason, 0) + 1
                n_skipped += 1
                log.info("Zone %d (%s): skip — %s",
                         zid, zone["name"], reason)
                continue
    ...
    def safety_check(self):
        ...
        for zone_id, active in list(self._active.items()):
            ...
            if elapsed > self.valve_timeout:
                log.warning("Safety timeout: Zone %d open for %ds — forcing close",
                            zone_id, int(elapsed))
    ...
    def update_daily_balances(self):
        ...
        Each day: balance -= ET₀ × Kc  (demand)
                  balance += rain_mm    (credit)
                  balance += irrigation_mm (credit, from watering events × precip rate)
        ...
        log.info("Updating soil water balances for %s (ET₀=%.2fmm, rain=%.1fmm)",
        ...
        for zone in self.config["zones"]:
            ...
            if prev and prev["date"] != today:
                ...
                balance = taw_mm  # first entry — assume field capacity

            # Crop coefficient for current season
            kc = zone["kc"][season_idx] if season_idx >= 0 else 0
            etc_mm = et0 * kc

            # Irrigation applied today (from watering events × precip rate)
    ...
    def save_daily_forecast_snapshot(self):
        ...
        log.info("Saving forecast snapshot for %s (ET₀=%.2fmm, rain_fcst=%.1fmm)",
