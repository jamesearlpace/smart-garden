...
# ===== Archived 2026-06-12 from main journey (older dated session logs) =====
...
- **Sensor power-gate (battery savings):** soil sensors currently powered 24/7 (~5 mA each = ~20 mA continuous, no gating). Add a GPIO/MOSFET to switch sensor VCC on only ~1s before `readSensors()`, then off — drops average sensor draw from ~20 mA continuous to near-zero (~1s every 5 min). Reuse the existing valve `POWER_GATE_PIN` pattern + `GATE_SETTLE_MS` settle delay; wrap the `analogRead` loop in `readSensors()`. Needs: a free GPIO + confirm sensor VCC rail (3.3V vs 5V). Bonus: a dead/shorted sensor (e.g. pin 32 stuck-low) can bleed elevated current 24/7 — gating eliminates that parasitic drain too.
