...
| 22 | Next Watering banner STILL not showing after fix #20 | 6 (below MAD) | Banner invisible despite code looking correct | Below-MAD check was nested inside forecast gate | ✅ Fixed: moved below-MAD check outside forecast gate |
| 23 | Next Watering banner STILL not showing after fixes #17, #20, #22 | 6 (below MAD) | 3 fix attempts failed. Banner div exists in HTML, JS function should set display:flex, but it never appears. | Moved call to refresh() scope (fix #22) but still not showing. Likely a JS runtime error in `updateNextWateringBanner` that silently prevents execution — the `.catch` in the Promise chain would swallow it. Need to wrap the call in try/catch with visible error reporting, or inline the banner logic directly in refresh() to eliminate all indirection. | Open — needs try/catch wrapper or inline implementation |
