...
| 20 | Next Watering banner not showing | 6 (below MAD) | Zone at 43.2% < MAD 50%, banner should say "needs water now — next 4 AM" but is not visible | Bug #17 fix added the `lastMoisture <= madPct` branch, code looks correct, banner.style.display='flex' should fire. Either: (a) `liveData.forecast_7day` is empty array (weather cache cold on first API call), (b) JS error prevents function from completing, or (c) `data.isRealData` lost during array manipulation. Needs browser console debugging. | Open — needs JS console trace to pinpoint |
| 21 | 30d range includes forecast, shortens visible history | ALL | "30d" shows May 10–Jun 8 instead of May 3–Jun 2 + forecast | `tMin = tMax - 30 days` where `tMax` = end of forecast (Jun 8), so 30d window starts 30 days before forecast end, not 30 days before today. User loses 7 days of history to forecast padding. | Open — fix: calculate `tMin` from "Now" not from end of data |
