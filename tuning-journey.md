...
| 22 | Next Watering banner STILL not showing after fix #20 | 6 (below MAD) | Banner invisible despite code looking correct — below-MAD check is outside forecast gate, `banner.style.display='flex'` should fire | Two possible causes under investigation: (a) JS error earlier in `buildChart` prevents `updateNextWateringBanner` from being called, (b) `data.isRealData` is `undefined` because the property was lost during array operations. Debug console.log added — awaiting browser console output from user. | Open — console.log deployed, awaiting user F12 trace |
