...
| 30 | Predicted sprinkler bar time changes when mouse moves | ALL | Hovering the precip chart shows a predicted sprinkler bar that jumps to different times as the mouse moves. Tooltip shows "Tue 5:00 PM" then "Wed 4:15 AM" etc. for the same predicted watering event. The time shown in the tooltip doesn't match the banner's "Wednesday Jun 3 ~4 AM". | The predicted sprinkler bars are generated as individual 15-min data points across the entire 4-7 AM window (12 bars per predicted watering). Chart.js `mode: 'index'` tooltip snaps to the nearest data point at the mouse's x position, so moving the mouse across the bar cluster shows different 15-min slots. Additionally, the `forecastSprinklerBars` dataset uses `p.sprinkler * rd / 100` which maps to many sparse points — the tooltip finds the nearest one regardless of whether it's visually under the cursor. **The core issue:** predicted watering should be ONE bar per day (total inches), not 12 separate 15-min bars. | Open — consolidate predicted sprinkler into one bar per watering day (sum of 4-7 AM window), positioned at 4 AM with a fixed-width bar. |
